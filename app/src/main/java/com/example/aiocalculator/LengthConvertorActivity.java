...
public class LengthConvertorActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    //    Spinner Data
    String[] data = {"meter", "Kilometer", "Centimeter", "Millimeter", "Inch", "Yard", "Feet", "Mile"};
    //    Calculation Values corresponding to spinner selected item
    String[] dataSymbols = {"[m]", "[km]", "[cm]", "[mm]", "[inch]", "[yd]", "[ft]", "[mile]"};
    ...
    private void initClickListeners() {
        ...
        tVDataConv1.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                ClipboardManager cm = (ClipboardManager)LengthConvertorActivity.this.getSystemService(Context.CLIPBOARD_SERVICE);
                String temp = tVDataConv1.getText().toString().trim();
                ClipData clipData = ClipData.newPlainText("copied to clipboard",temp);
                cm.setPrimaryClip(clipData);
                Toast.makeText(LengthConvertorActivity.this, "Result Copied", Toast.LENGTH_SHORT).show();
                return true;
            }
        });
    ...
    public void convOnClick(View v){
        switch (v.getId()){
            case R.id.btnC:{
                ...
                boolean flag = false;
                if(temp.length() >= 16){
                    Toast.makeText(LengthConvertorActivity.this, "Input number is too long", Toast.LENGTH_SHORT).show();
                    break;
                }
    ...
    private void initData() {
        tVTitle.setText("Length Convertor");
