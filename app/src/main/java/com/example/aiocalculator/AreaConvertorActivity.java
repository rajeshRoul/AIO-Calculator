...
public class AreaConvertorActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {

    //    Spinner Data
    String[] data = {"Square meter", "Square centimeter", "Square millimeter", "Are", "Hectare", "Acre", "Square Kilometer"};
    //    Calculation Values corresponding to spinner selected item
    String[] dataSymbols = {"[m2]", "[cm2]", "[mm2]", "[are]", "[ha]", "[acre]", "[km2]"};
    ...
    private void initClickListeners() {
        ...
        tVDataConv1.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                ClipboardManager cm = (ClipboardManager)AreaConvertorActivity.this.getSystemService(Context.CLIPBOARD_SERVICE);
                String temp = tVDataConv1.getText().toString().trim();
                ClipData clipData = ClipData.newPlainText("copied to clipboard",temp);
                cm.setPrimaryClip(clipData);
                Toast.makeText(AreaConvertorActivity.this, "Result Copied", Toast.LENGTH_SHORT).show();
                return true;
            }
        });
    ...
    public void convOnClick(View v){
        switch (v.getId()){
            case R.id.btnC:{
                ...
                boolean flag = false;
                if(temp.length() >= 16){
                    Toast.makeText(AreaConvertorActivity.this, "Input number is too long", Toast.LENGTH_SHORT).show();
                    break;
                }
    ...
    private void initData() {
        tVTitle.setText("Area Convertor");
