...
public class VolumeConvertorActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {

    //    Spinner Data
    String[] data = {"Cubic millimeter", "Cubic centimeter", "Cubic meter", "Cubic kilometer", "Millilitre", "Litre"};
    //    Calculation Values corresponding to spinner selected item
    String[] dataSymbols = {"[mm3]", "[cm3]", "[m3]", "[km3]", "[ml]", "[l]"};
    ...
    private void initClickListeners() {
        ...
        tVDataConv1.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                ClipboardManager cm = (ClipboardManager)VolumeConvertorActivity.this.getSystemService(Context.CLIPBOARD_SERVICE);
                String temp = tVDataConv1.getText().toString().trim();
                ClipData clipData = ClipData.newPlainText("copied to clipboard",temp);
                cm.setPrimaryClip(clipData);
                Toast.makeText(VolumeConvertorActivity.this, "Result Copied", Toast.LENGTH_SHORT).show();
                return true;
            }
        });
    ...
    public void convOnClick(View v){
        switch (v.getId()){
            case R.id.btnC:{
                ...
                boolean flag = false;
                if(temp.length() >= 16){
                    Toast.makeText(VolumeConvertorActivity.this, "Input number is too long", Toast.LENGTH_SHORT).show();
                    break;
                }
    ...
    private void initData() {
        tVTitle.setText("Volume Convertor");
