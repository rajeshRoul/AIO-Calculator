...
public class EnergyConvertorActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {

    //    Spinner Data
    String[] data = {"Joule", "Electronovolt", "Kiloelectronovolt", "Megaelectronovolt", "Gigaelectronovolt", "Teraelectronovolt"};
    //    Calculation Values corresponding to spinner selected item
    String[] dataSymbols = {"[J]", "[eV]", "[keV]", "[MeV]", "[GeV]", "[TeV]"};
    ...
    private void initClickListeners() {
        ...
        tVDataConv1.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                ClipboardManager cm = (ClipboardManager)EnergyConvertorActivity.this.getSystemService(Context.CLIPBOARD_SERVICE);
                String temp = tVDataConv1.getText().toString().trim();
                ClipData clipData = ClipData.newPlainText("copied to clipboard",temp);
                cm.setPrimaryClip(clipData);
                Toast.makeText(EnergyConvertorActivity.this, "Result Copied", Toast.LENGTH_SHORT).show();
                return true;
            }
        });
    ...
    public void convOnClick(View v){
        switch (v.getId()){
            case R.id.btnC:{
                ...
                boolean flag = false;
                if(temp.length() >= 16){
                    Toast.makeText(EnergyConvertorActivity.this, "Input number is too long", Toast.LENGTH_SHORT).show();
                    break;
                }
    ...
    private void initData() {
        tVTitle.setText("Energy Convertor");
